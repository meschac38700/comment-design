...
export const loadingComment = (state) => {
	state.forEach((comment_data) => {
		...
		let parent_c = new Comment(
			`${parent.author__first_name} ${parent.author__last_name.toUpperCase()}`,
			parent.content,
			parent.nbr_vote,
			date_added,
 ...
 ******************************************************/
export function hide_show_child_comment_text(el) {
...
export function hide_or_add_show_children_btn(parent_comment = null) {
